...
public class PlayField extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        world = new TileMap(140, 140, Color.DARKBLUE);
        viewPort = new ViewPort(world, field);
        ...
        scrollbarX.valueProperty().addListener(new ChangeListener<Number>() {
            public void changed(ObservableValue<? extends Number> ov,
                Number old_val, Number new_val) {
                    viewPort.modify(new_val.doubleValue(), 0, 0, 0);
            }
        });
        ...
        scrollbarY.valueProperty().addListener(new ChangeListener<Number>() {
            public void changed(ObservableValue<? extends Number> ov,
                Number old_val, Number new_val) {
                    viewPort.modify(0, new_val.doubleValue(), 0, 0);
            }
        });
        ...
        scrollbarZ.valueProperty().addListener(new ChangeListener<Number>() {
            public void changed(ObservableValue<? extends Number> ov,
                Number old_val, Number new_val) {
                    viewPort.modify(0, 0, new_val.doubleValue(), new_val.doubleValue());
            }
        });
        ...
        chkBoxAutoZoom.selectedProperty().addListener(new ChangeListener<Boolean>() {
            public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
                viewPort.setAutoZoomEnabled(newValue);
            } 
        });
        ...
        ChangeListener<Number> SceneWidthResizeListener = new ChangeListener<Number>() {
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
                ...
                viewPort.resize(scrollbarX.getPrefWidth() - (2 * scrollbarX.getLayoutX()), 0);
            }
        };
        ...
        ChangeListener<Number> SceneHeightResizeListener = 
            (ObservableValue<? extends Number> observable, Number oldValue, Number newValue) -> {
                ...
                viewPort.resize(0, scrollbarY.getPrefHeight() - (2 * scrollbarY.getParent().getLayoutY()));
        };
        scene.heightProperty().addListener(SceneHeightResizeListener);
             
        // Handle ViewPort Properties Modify Event:
        viewPort.modifyEvent.addListener(new CallbackEvent.Listener<ViewPort>() {
        ...
        viewPort.resizeEvent.addListener(new CallbackEvent.Listener<ViewPort>() {
            public void callback_Event(ViewPort vp) {
                field.setWidth(vp.width);
                field.setHeight(vp.height);
                /// System.out.println("resize: " + root.getWidth() + ", " + root.getHeight());
            }   
        });
        
        // Handle ViewPort Redraw Event:
        viewPort.redrawEvent.addListener(new CallbackEvent.Listener<ViewPort>() {
            public void callback_Event(ViewPort vp) {
                world.renderTiles(gc, vp);
                /// System.out.println("redraw");
            }   
        });
        
        // Handle ViewPort updateScrollbarX Event:
        viewPort.updateScrollbarX.addListener(new CallbackEvent.Listener<ViewPort>() {
            public void callback_Event(ViewPort vp) {
        ...
        viewPort.updateScrollbarY.addListener((ViewPort vp) -> {
        ...
        Image windowIcon = imgHelper.getImgFromResFile(
                getPackageID(this) + "/../../app/resource/life.png", 48, 48, true);
        if (windowIcon != null) {
